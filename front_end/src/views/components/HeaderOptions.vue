...
<template>
  ...
  <div class="top-icon-wrap">
    ...
    <el-button style="margin-right: 1rem" type="primary" plain @click="saveAsTemplate">保存模板</el-button>
...
async function downloadFrontend() {
  ...
  try {
    ...
    useNotification('导出失败', '请稍后重试', { type: 'error' })
    emit('change', { downloadPercent: 0, downloadText: '导出失败' })
  } finally {
    state.loading = false
  }
}
...
async function saveAsTemplate() {
  ...
  try {
    ...
    let cover = ''
    const poster = await canvasImage.value?.createPoster()
    if (poster?.blob) {
      cover = await blobToDataUrl(poster.blob)
    }
    const payload = {
      title: state.title || '未命名模板',
      data: JSON.stringify(dLayouts.value),
      width: dPage.value.width,
      height: dPage.value.height,
      cover,
    }
    const res: any = await api.home.saveTemp(payload)
    if (res?.stat !== 0) {
      useNotification('保存成功', '模板已加入模板库')
    } else {
      useNotification('保存失败', res?.msg || '请稍后重试', { type: 'error' })
    }
  } catch (error: any) {
    console.error('Save template failed', error)
    useNotification('保存失败', error?.message || '请稍后重试', { type: 'error' })
  } finally {
    controlStore.setShowMoveable(true)
    state.loading = false
  }
